<template>
    <div>
        <div class="title">
            ...
            <h1>Tabla de oraciones</h1>
        </div>
      <div class="container">
        <div v-if="mensaje" class="mensaje">{{ mensaje }}</div>
        <v-data-table :headers="headers" :items="oraciones" item-key="id" class="tabla-palabras">
          <template slot="item" slot-scope="{ item }">
            <tr>
              ...
              <td class="text-left">
                <v-btn class="editar-button" @click="editarOracion(item[0])">
                  <v-icon>mdi-pencil</v-icon>
                </v-btn>
              </td>
              <td class="text-left">
                <v-btn class="eliminar-button" @click="eliminarOracion(item[0])">
  ...
  export default {
    ...
    methods: {
      obteneroraciones() {
        axios
          .get('https://edutlasdeveloper.pythonanywhere.com/api/showtableo')
          .then((response) => {
      ...
      editarOracion(id) {
        this.$router.push(`editar-oracion/editar/${id}`);
      },
      eliminarOracion(id) {
        axios
          .delete(`https://edutlasdeveloper.pythonanywhere.com/api/oraciones/${id}`)
          .then((response) => {
      ...
      openModal(id) {
        axios
          .get(`https://edutlasdeveloper.pythonanywhere.com/api/oraciones/${id}/image`)
          .then((response) => {
      ...
      atras() {
        this.$router.push("/Campos/ingresar-oracion");
      }
    }
  };
  </script>
